        ...
        <div class="tile">
          <div class="tile-body">
            ...
            <table class="table table-hover table-bordered js-copytextarea" cellpadding="0" cellspacing="0" border="0"
              ...
              <tbody>
              <?php 
                    $sql = "SELECT tb_canho_phong.*, tb_toanha.ten_toanha 
                    FROM `tb_canho_phong`
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <span class="text-muted">
                      <br>Tòa nhà: <?php echo $row['ten_toanha']; ?> </span>
                    <span class="text-muted">
                    <br>Tầng: Tầng  <?php echo $row['tang']; ?> </span>
                </td>
                  ...
                  <td>
                      <?php 
                          if($row['trangthai_canho_phong'] == 1){
                            echo '<span class="badge bg-success" style="font-size: 13px;"><b class="span_pending">Đã thuê</b></span>';
  ...
  $(document).ready(function () {
    ...
    function initializeDropdowns(buildingId, roomId) {
        ...
        building.onchange = function () {
          room.disabled = false;          
          // Lấy giá trị từ biến JavaScript
          var idbuilding = building.value;

            // Gửi giá trị đến máy chủ PHP bằng Fetch API
            fetch('danh_sach_canho_phong.php', {
              method: 'POST',
              body: JSON.stringify({ idbuilding: idbuilding }),
              headers: {
            ...
            .then(data => {
              console.log(data); // Xử lý dữ liệu trả về từ máy chủ
            })
            .catch(error => {
              console.error(error);
            });
            <?php
              ...
              $sqlSelect = "SELECT * FROM tb_toanha WHERE id_toanha = $idbuilding";
              $querySelect = mysqli_query($conn, $sqlSelect);
              if(mysqli_num_rows($querySelect) > 0){
                while ($rowSelect = mysqli_fetch_array($querySelect)) {
                  $index = 0;
                  echo "wards.options[".json_encode($index)."] = new Option(w.Name, w.Id = w.Name);";
                  $index++;
                }
              }
            ?>
