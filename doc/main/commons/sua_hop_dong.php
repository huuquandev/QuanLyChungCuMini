    ...
    <div class="app-title">
      <ul class="app-breadcrumb breadcrumb side">
        <li class="breadcrumb-item active"><a href="#"><b>
          Sửa dân cư
          <?php
          $sql="SELECT h.*,t.ten_hien_thi,ph.ten_canho_phong from `tb_hopdong` AS h 
          ...
          $query = mysqli_query($conn, $sql);
              ...
              // Hiển thị tên
              echo  $ten_hien_thi."- "." với id hợp đồng:".$_GET['id'];
          ?>
        </b></a></li>
      </ul>
      <div id="clock"></div>
        ...
        <div class="tile">
          ...
          <?php
                                ...
                                $query0 = mysqli_query($conn, $sql0);
                                    ...
                                    $id=$row['id'];
                                    $bat_dau = $row['bat_dau'];
                                    $ket_thuc=$row['ket_thuc'];
                                    $id_dan_cu=$row['id_dan_cu'];
                                    $id_can_ho_phong=$row['id_can_ho_phong'];
                                    $tong=$row['tong'];
                                      ...
                                      <div dir="ltr" class="v-select vs--single vs--searchable" id="province">
                                        <div id="vs33__combobox" role="combobox" aria-expanded="false" aria-owns="vs33__listbox" aria-label="Search for option" class="vs__dropdown-toggle">
                                          ...
                                          <select name="name_can_ho" id="name_can_ho"aria-autocomplete="list" aria-labelledby="vs33__combobox" aria-controls="vs33__listbox" autocomplete="off" class="vs__search"  >
                                                <?php 
                                                $sql1="SELECT c.`ten_canho_phong`,c.`id_canho_phong`,t.`ten_toanha`,h.`id` FROM `tb_canho_phong` AS c INNER JOIN `tb_toanha` AS t INNER JOIN `tb_hopdong` AS h ON t.`id_toanha` = c.`id_toanha`AND c.`id_canho_phong`=h.`id_can_ho_phong` WHERE h.`id`=$id";
                                                ...
                                                INNER JOIN tb_hopdong AS h ON c.id_canho_phong = h.id_can_ho_phong
                                                WHERE h.id_can_ho_phong != $id_can_ho_phong;";
                                                $query=mysqli_query($conn,$sql);
                                    ...
                                    <div>
                                      <div dir="ltr" class="v-select vs--single vs--searchable" id="province">
                                        <div id="vs33__combobox" role="combobox" aria-expanded="false" aria-owns="vs33__listbox" aria-label="Search for option" class="vs__dropdown-toggle">
                                          ...
                                          <?php
                                            $sql2 = "SELECT `cccd`, `ten_hien_thi` FROM `tb_dancu` WHERE `cccd` = " . $id_dan_cu;
                                            $query2 = mysqli_query($conn, $sql2);
                                            $row2 = mysqli_fetch_assoc($query2);
                                            echo '<option value="' . $row2['cccd'] . '">' . $row2['ten_hien_thi'] . '-' . $row2['cccd'] . '</option>';
...
// Specify the relative path to the desired upload directory
    ...
    $query4 = mysqli_query($conn, $sql4);
        ...
        // Update the database
        $content = updateSql('id_can_ho_phong', $name_can_ho, $_GET['id']);
        $content .= updateSql('bat_dau', $ngaybatdau, $_GET['id']);
        $content .= updateSql('ket_thuc', $ngayketthuc, $_GET['id']);
