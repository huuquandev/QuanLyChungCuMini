						...
						<div class="row">
							<div class="form-group col-md-4">
								...
								<select class="form-control" id="dv" name="dv" required >
									...
									<option value="<?php echo $value['id']; ?>"><?php echo $value['id'] . " - " . $value['TenDV']; ?></option>
									<?php } ?>
								</select>
							</div>
							<div class="form-group col-md-4">
							...
							<div class="form-group col-md-4">
								...
								<input class="form-control" type="number" id="loai" name="loai" required value="<?php echo $dataHoaDon['loai'];?>">
							</div>
							<div class="form-group col-md-4">
							...
							<label for="exampleSelect1" class="control-label">Ngày thanh toán</label>
								<input class="form-control" type="datetime-local" id="ngaythanhtoan" name="ngaythanhtoan" disabled required value="<?php echo date('Y-m-d\TH:i', strtotime($dataHoaDon['ngaythanhtoan']));?>">
							</div>
							<div class="form-group col-md-4">
							...
							<select class="form-control" id="tinhtrang" name="tinhtrang" required disabled>
								<option value="<?php echo $dataHoaDon['tinhtrang']; ?>"><?php echo $dataHoaDon['tinhtrang']; ?></option>
							</select>
							</div>                
						</div>
					...
					<?php 
						if(isset($_POST['btnSave'])){
							...
							if (empty($errors)) {
								...
								if(UpdateHoaDon($id, $dichVu, $hopDong, $loai, $ngayHetHan, $ngayTao, $gia)){
									echo "<script>alert('Lưu dữ liệu thành công!');</script>";
								} else {
									echo "<script>alert('Lưu dữ liệu thất bại!');</script>";
								}
							} else {
